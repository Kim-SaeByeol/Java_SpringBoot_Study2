...
public class EncryptUtil {
     ...
     * 암호화 된 값만 보고 일반적인 비밀번호에 대한 값을 쉽게 예측이
     * 가능함에 따라, 암호화 할 때 암호화 되는 값에 추가적인 문자열을 붙여서 함께
     * 암호화를 진행함
     *
     */
    ...
    //초기 백터 (16바이트 크기를 가지며, 16바이트 단위로 암호화 시
    //암호화 할 총 길이가 16바이트가 되지 못하면뒤에 추가하는 바이트

    final static byte[] ivBytes = {0x00, 0x00,
            0x00, 0x00,
            0x00, 0x00,
            0x00, 0x00,
            0x00, 0x00,
    ...
    final static String key = "PolyTechnic12345";   //16글자(영문 = 1바이트)
    ...
    public static String encHashSHA256(String str) throws Exception {
        ...
        String planText = addMessage + str; //암호화 시킬 값에 보안강화를 위해 임의 값을 추가함

        try {
            //자바는 기본적으로 표준 알고리즘을 java.security 패키지를 통해 제공함
    ...
    /**
     * AES128 CBC 암호화 함수
     * 128은 암호화 키 길이를 의미함. 128 비트 = 16바이트
     */
